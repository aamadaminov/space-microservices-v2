...
func (c *KafkaConsumer) Start() error {
	if len(c.handlers) == 0 {
		log.Println("No handlers registered, consumer will not start")
		return nil
	}
	...
	log.Printf("Starting consumer: topics %s, group %s", topics, c.groupID)
	...
	for message := range claim.Messages() {
		if handler, ok := h.handlers[message.Topic]; ok {
			if err := handler(message); err != nil {
				log.Fatalf("Error handling message: topic %s, partition %d, offset %d, %v\n", message.Topic, message.Partition, message.Offset, err)
			}
		}
